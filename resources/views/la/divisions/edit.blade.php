...
@section("contentheader_title")
	<a href="{{ url(config('laraadmin.adminRoute') . '/divisions') }}">Division</a> :
@endsection
			...
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($division, ['route' => [config('laraadmin.adminRoute') . '.divisions.update', $division->id ], 'method'=>'PUT', 'id' => 'division-edit-form']) !!}
					...
					<div class="form-group">
						{!! Form::submit( 'Update', ['class'=>'btn btn-success']) !!} <button class="btn btn-default pull-right"><a href="{{ url(config('laraadmin.adminRoute') . '/divisions') }}">Cancel</a></button>
					</div>
				{!! Form::close() !!}
			</div>
		</div>
